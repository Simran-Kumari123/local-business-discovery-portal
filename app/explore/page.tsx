...
function loadGoogleMapsScript(apiKey: string): Promise<void> {
    return new Promise((resolve, reject) => {
        ...
        if ((window as unknown as Record<string, unknown>).google && (window as unknown as Record<string, Record<string, unknown>>).google.maps) {
            resolve()
            return
        }
...
function ExploreContent() {
    ...
    // Load Google Maps SDK when we have the key
    useEffect(() => {
        if (!apiKey) return
    ...
    useEffect(() => {
        ...
        // If we have user location, show marker
        if (userLocation) {
    ...
    const addUserLocationMarker = (map: google.maps.Map, loc: { lat: number; lng: number }) => {
    ...
    /* ═══ Get directions ═══ */
    const getDirections = (destination: { lat: number; lng: number }, destName: string) => {
        ...
        <div className="min-h-screen bg-secondary/30">
                ...
                <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 md:py-14">
                    <div className="text-center text-white mb-6">
                        ...
                        <p className="text-emerald-100 text-lg max-w-2xl mx-auto">
                            Search any business type + location — see real results on the map with directions.
                        </p>
                    </div>
                    ...
                    <div className="flex flex-wrap items-center justify-center gap-2 mt-5">
                        ...
                        {quickSearches.map((qs) => (
                            <button
                                ...
                                className={`px-3 py-1 text-xs font-semibold rounded-full transition-all duration-150 ${query === qs.query
            ...
            <section className="max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8 py-6">
                ...
                <div className="flex items-center justify-between mb-4">
                    <div className="flex items-center gap-4">
                        <p className="text-sm text-muted-foreground">
                            {loading ? 'Searching...' : hasSearched ? (
                                <>
                                    <span className="font-bold text-foreground">{places.length}</span> place{places.length !== 1 && 's'} found
                    ...
                    <div className="hidden md:flex items-center gap-1 bg-card border border-border rounded-xl p-1">
                        <button
                            ...
                            className={`flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold rounded-lg transition-all ${viewMode === 'split'
                        ...
                        <button
                            ...
                            className={`flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold rounded-lg transition-all ${viewMode === 'map'
                                ? 'bg-primary text-primary-foreground shadow-sm'
                                : 'text-muted-foreground hover:text-foreground'
                                }`}
                        >
                ...
                <div className={`flex gap-6 ${viewMode === 'map' ? 'flex-col' : ''}`}>
                    {/* ── RESULTS LIST ── */}
                    {viewMode !== 'map' && (
                        <div className={`${viewMode === 'split' ? 'w-1/2' : 'flex-1'} space-y-3 max-h-[calc(100vh-200px)] overflow-y-auto pr-2`}>
                            {loading ? (
                                <div className="flex flex-col items-center justify-center py-20">
                                    <Loader2 className="w-10 h-10 animate-spin text-emerald-500 mb-4" />
                                    <p className="text-muted-foreground font-medium">Searching real businesses...</p>
                                </div>
                            ) : places.length > 0 ? (
                                places.map((place, index) => (
                                    <div
                                        ...
                                        className={`bg-card border rounded-2xl overflow-hidden hover:shadow-lg hover:-translate-y-0.5 transition-all duration-300 cursor-pointer group ${selectedPlace?.placeId === place.placeId
                                        ...
                                        <div className="flex">
                                            ...
                                            <div className="flex-1 p-4 min-w-0">
                                                <div className="flex items-start justify-between gap-2 mb-1">
                                                    ...
                                                    {place.openNow !== null && (
                                                        <span className={`shrink-0 px-2 py-0.5 text-[10px] font-bold rounded-full ${place.openNow
                                                        ...
                                                        <div className="flex items-center gap-1">
                                                            <div className="flex items-center gap-0.5">
                                                                {[1, 2, 3, 4, 5].map((star) => (
                                                                    <Star
                                                                        key={star}
                                                                        className={`w-3.5 h-3.5 ${star <= Math.round(place.rating)
                                ...
                                <div className="text-center py-16">
                                    ...
                                    <p className="text-muted-foreground text-sm max-w-md mx-auto">
                                        Try searching with different keywords or adding a location like &quot;Ahmedabad&quot;.
                                    </p>
                                </div>
                            ) : (
                                <div className="text-center py-16">
                                    ...
                                    <p className="text-muted-foreground text-sm max-w-md mx-auto mb-4">
                                        Try searching &quot;restaurants near me&quot; or &quot;gyms in Ahmedabad&quot;
                                    </p>
                                    <button
                                        onClick={getUserLocation}
                                        disabled={locating}
                    ...
                    {/* ── MAP ── */}
                    {viewMode !== 'list' && (
                        <div className={`${viewMode === 'split' ? 'w-1/2' : 'flex-1'} sticky top-24`}>
                            <div className="bg-card border border-border rounded-2xl overflow-hidden shadow-sm" style={{ height: viewMode === 'map' ? 'calc(100vh - 250px)' : 'calc(100vh - 200px)' }}>
                                {!apiKey ? (
                                    <div className="w-full h-full flex items-center justify-center bg-secondary/50">
                                        <div className="text-center p-8">
                                            <MapIcon className="w-16 h-16 text-muted-foreground/30 mx-auto mb-4" />
                                            <p className="text-muted-foreground font-medium mb-2">Google Maps API key not configured</p>
                                            <p className="text-xs text-muted-foreground">Add <code className="bg-secondary px-1.5 py-0.5 rounded">NEXT_PUBLIC_GOOGLE_MAPS_API_KEY</code> to your .env file</p>
                                        </div>
                                    </div>
                                ) : (
                ...
                {(selectedPlace || detailsLoading) && (
                    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/40 backdrop-blur-sm" onClick={() => setSelectedPlace(null)}>
                        <div
                                ...
                                <>
                                    ...
                                    {selectedPlace.photos.length > 0 && (
                                        <div className="relative h-52 sm:h-64 overflow-hidden rounded-t-3xl sm:rounded-t-3xl">
                                            ...
                                            <div className="absolute bottom-4 left-5 right-5 text-white">
                                                <h2 className="text-2xl font-bold mb-1" style={{ fontFamily: 'var(--font-sora), sans-serif' }}>
                                                    {selectedPlace.name}
                                                </h2>
                                                <div className="flex items-center gap-3 text-sm">
                                                    {selectedPlace.rating > 0 && (
                                                        <span className="flex items-center gap-1">
                                                            ...
                                                            <span className="text-white/80">({selectedPlace.totalRatings})</span>
                                                        </span>
                                                    )}
                                                    {selectedPlace.openNow !== null && (
                                                        <span className={`px-2 py-0.5 text-xs font-bold rounded-full ${selectedPlace.openNow
                                    ...
                                    {!selectedPlace.photos.length && (
                                        <div className="pt-6 px-6 pb-2">
                                            <h2 className="text-2xl font-bold text-foreground" style={{ fontFamily: 'var(--font-sora), sans-serif' }}>
                                                {selectedPlace.name}
                                            </h2>
                                        </div>
                                    )}
                                    ...
                                    <div className="p-6 space-y-5">
                                        ...
                                        <div className="flex gap-2 flex-wrap">
                                            <button
                                                onClick={() => getDirections(selectedPlace.location, selectedPlace.placeId)}
                                            ...
                                            {selectedPlace.phone && (
                                                ...
                                                >
                                                    <Phone className="w-4 h-4 text-blue-500" /> {selectedPlace.phone}
                                                </a>
                                            )}
                                        ...
                                        {selectedPlace.reviews.length > 0 && (
                                            <div>
                                                <h4 className="text-sm font-semibold text-foreground mb-3">Reviews from Google</h4>
                                                            ...
                                                            <div className="flex items-center gap-3 mb-2">
                                                                ...
                                                                <div>
                                                                    ...
                                                                    <div className="flex items-center gap-1">
                                                                        ...
                                                                        <span className="text-[10px] text-muted-foreground ml-1">{review.time}</span>
                                                                    </div>
                                                                </div>
                                                            </div>
